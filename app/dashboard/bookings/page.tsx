"use client";

import { useEffect, useState } from "react";
import { useAuth } from "@/context/AuthContext";
import { Booking } from "@/types/booking";
import BookingFormModal from "@/components/bookings/BookingModal";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { deleteApi, getApi, putApi } from "@/lib/api";
import { CircleCheck, CircleX, Pencil, Trash2 } from "lucide-react";
import toast from "react-hot-toast/headless";
import DropdownMenu from "@/components/ui/dropdown-menu";
import {
  Table,
  TableHeader,
  TableBody,
  TableHead,
  TableRow,
  TableCell,
} from "@/components/ui/table";

export default function BookingPage() {
  const { loading } = useAuth();

  const [Booking, setBooking] = useState<Booking[]>([]);
  const [open, setOpen] = useState(false);
  const [selectedBooking, setSelectedBooking] = useState<Booking | null>(null);

  // 🔥 Pagination + Search state
  const [page, setPage] = useState(1);
  const [limit] = useState(10);
  const [totalPages, setTotalPages] = useState(1);
  const [search, setSearch] = useState("");

  useEffect(() => {
    fetchBooking();
  }, [page, search]);

  const fetchBooking = async () => {
    try {
      const res = await getApi(
        `/bookings?page=${page}&limit=${limit}&search=${search}`,
      );
      setBooking(res.data || []);
      setTotalPages(res.pagination?.totalPages || 1);
    } catch (error) {
      console.error("Fetch error");
    }
  };

  const handleEdit = (Booking: Booking) => {
    setSelectedBooking(Booking);
    setOpen(true);
  };

  const handleAdd = () => {
    setSelectedBooking(null);
    setOpen(true);
  };

  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    const res = await putApi(`/bookings/${id}/change-status`, {
      isActive: !currentStatus,
    });
    if (res.success) {
      toast.success(res.message || "Status updated successfully"); // refresh list
      fetchBooking();
    } else {
      toast.error("Failed to update status");
    }
  };
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this Booking?")) return;
    const res = await deleteApi(`/bookings/${id}`);
    if (res.success) {
      toast.success(res.message || "Booking deleted successfully"); // refresh list
      fetchBooking(); // refresh list
    } else {
      toast.error("Failed to delete");
    }
  };

  if (loading) return <div>Loading...</div>;

  return (
    <div>
      {/* HEADER */}
      <div className="flex justify-between mb-6">
        <h1 className="text-2xl font-bold">Booking</h1>
        <Button onClick={handleAdd}>Add Booking</Button>
      </div>

      {/* 🔍 SEARCH */}
      <div className="mb-4">
        <Input
          placeholder="Search Booking..."
          value={search}
          onChange={(e) => {
            setPage(1);
            setSearch(e.target.value);
          }}
        />
      </div>

      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>#</TableHead>
            <TableHead>Booking Number</TableHead>
            <TableHead>Customer</TableHead>
            <TableHead>Room</TableHead>
            <TableHead>Branch</TableHead>
            <TableHead>Adult</TableHead>
            <TableHead>Child</TableHead>
            <TableHead>Amount</TableHead>
            <TableHead>Discount</TableHead>
            <TableHead>Tax</TableHead>
            <TableHead>Total Amount</TableHead>
            <TableHead>Check-in Date</TableHead>
            <TableHead>Check-out Date</TableHead>
            <TableHead>Special Requests</TableHead>
            <TableHead>Status</TableHead>
            <TableHead>Created At</TableHead>
            <TableHead className="text-right">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {Booking.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={9}
                className="text-center text-gray-400 py-10"
              >
                No Bookings found.
              </TableCell>
            </TableRow>
          ) : (
            Booking.map((b, index) => (
              <TableRow key={b.id}>
                <TableCell className="text-gray-400">{index + 1}</TableCell>
                <TableCell className="font-medium text-gray-800">
                  {b.bookingNumber}
                </TableCell>
                <TableCell className="text-gray-600">
                  {b.customer?.name}
                  <br />
                  {b.customer?.email}
                  <br />
                  {b.customer?.phone}
                </TableCell>
                <TableCell className="text-gray-600">
                  {b.room?.roomNumber}
                </TableCell>
                <TableCell className="text-gray-600">
                  {b.branch?.name}
                </TableCell>
                <TableCell className="text-gray-600">{b.adults}</TableCell>
                <TableCell className="text-gray-600">{b.children}</TableCell>
                <TableCell className="text-gray-600">{b.totalAmount}</TableCell>
                <TableCell className="text-gray-600">{b.discount}</TableCell>
                <TableCell className="text-gray-600">{b.tax}</TableCell>
                <TableCell className="text-gray-600">{b.finalAmount}</TableCell>
                <TableCell className="text-gray-600">{b.checkInDate}</TableCell>
                <TableCell className="text-gray-600">
                  {b.checkOutDate}
                </TableCell>
                <TableCell className="text-gray-600">
                  {b.specialRequests}
                </TableCell>
                <TableCell>
                  <span
                    className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium capitalize
    ${
      b.status === "confirmed"
        ? "bg-green-100 text-green-700"
        : b.status === "pending"
          ? "bg-yellow-100 text-yellow-700"
          : b.status === "cancelled"
            ? "bg-red-100 text-red-600"
            : b.status === "checked_in"
              ? "bg-blue-100 text-blue-700"
              : b.status === "checked_out"
                ? "bg-gray-100 text-gray-600"
                : "bg-gray-100 text-gray-500"
    }`}
                  >
                    {b.status?.replace(/_/g, " ")}
                  </span>
                </TableCell>

                <TableCell className="text-gray-500">
                  {new Date(b.createdAt).toLocaleDateString("en-IN", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
                  })}
                </TableCell>
                <TableCell className="text-right">
                  <DropdownMenu
                    items={[
                      {
                        label: "Edit",
                        icon: <Pencil size={14} className="text-blue-500" />,
                        onClick: () => handleEdit(b),
                      },
                      
                      {
                        label: "Delete",
                        icon: <Trash2 size={14} />,
                        onClick: () => handleDelete(b.id),
                        className: "text-red-600 hover:bg-red-50",
                      },
                    ]}
                  />
                </TableCell>
              </TableRow>
            ))
          )}
        </TableBody>
      </Table>

      {/* 📄 PAGINATION */}
      <div className="flex justify-center mt-6 gap-2">
        <Button disabled={page === 1} onClick={() => setPage((p) => p - 1)}>
          Prev
        </Button>

        <span className="px-4 py-2">
          Page {page} of {totalPages}
        </span>

        <Button
          disabled={page === totalPages}
          onClick={() => setPage((p) => p + 1)}
        >
          Next
        </Button>
      </div>

      {/* MODAL */}
      <BookingFormModal
        open={open}
        setOpen={setOpen}
        selectedBooking={selectedBooking}
        onSuccess={fetchBooking}
      />
    </div>
  );
}
